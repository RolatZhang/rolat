...
object Sender {
  def main(args: Array[String]): Unit = {
    ...
    val remoteHostPort = "127.0.0.1:2553"
    val remotePath = s"akka.tcp://Sys@$remoteHostPort/user/rcv"
    val totalMessages =  500000
    val burstSize =  5000
    val payloadSize =  100
  ...
  sealed trait Echo extends Serializable
...
class Sender(path: String, totalMessages: Int, burstSize: Int, payloadSize: Int) extends Actor {
  ...
  context.setReceiveTimeout(3000.millisecond)
  sendIdentifyRequest()
  ...
  def active(actor: ActorRef): Receive = {
    ...
    case Terminated(`actor`) =>
      println("远程服务已关闭Receiver terminated")
  ...
  def sendBatch(actor: ActorRef, remaining: Int): Int = {
    println(s"将要发送的数据总：$remaining,一次："+burstSize)
    val batchSize = math.min(remaining, burstSize)
    (1 to batchSize) foreach { x => actor ! x }
    remaining - batchSize
  }
}

...
import rolat.scala.test.ActorHierarchyExperiments.system
...
object MyActor extends App{
  ...
  val cleanUpActor=system.actorOf(Props[CleanUpActor],"CleanUpActor")
  val echoActor=system.actorOf(Props[EchoActor],"EchoActor")
  val myActor=system.actorOf(Props(new MyActor(echoActor,cleanUpActor)),"myActor")
  myActor ! "开始"
  try{
